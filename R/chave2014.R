...
chave2014 <- function(D, WD, H = NULL, coord = NULL, Dlim = NULL) {
  ...
  if (length(D) != length(WD)) {
    stop("D and WD have different lengths")
  }

  if (!is.null(H)) {
    if (length(D) != length(H)) {
      stop("H and WD have different lengths")
    }
    ...
    if (anyNA(H) & !anyNA(D)) {
      warning("There are some NA values in given heights. For those trees the function will return NA AGB,
               you may construct a height-diameter model to overcome that issue (see ?HDFunction and ?retrieveH)")
    }

    if (!is.null(coord)) {
      stop("Both height and coordinates are provided.")
    }
  }
  ...
  if (is.null(H) && is.null(coord)) {
    stop("You need to provide either H or coord for the Chave 2014 model")
  }
  ...
  if (!is.null(H)) {
    # Eq 4 from Chave et al. 2014 Global Change Biology
    AGB <- (0.0673 * (WD * H * D^2)^0.976) / 1000 
  } else {

    # If there is no height, but there are coordinates:
    if (is.null(dim(coord))) {
      coord <- as.matrix(t(coord))
    }

    # Fetch environmental index (E) in Chave et al. 2014
    E <- computeE(coord) 
  ...
  # Apply diameter limit if provided
  if (!is.null(Dlim)) {
    AGB[D < Dlim] <- 0
  }
