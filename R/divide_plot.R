...
#'   - `x_rel` and `y_rel` : the relative X-axis and Y-axis coordinates
#'   - `subplot_id` : the subplot associated
...
#' subplots <- divide_plot(rel_coord, proj_coord, grid_size = 100)
#'
#' tree_coord <- data.frame(x_proj = runif(50,0,200), y_proj = runif(50,0,200))
#' subplots <- divide_plot(rel_coord, proj_coord, grid_size = 100, tree_coord = tree_coord)
#'
...
divide_plot <- function(rel_coord, proj_coord = NULL, grid_size, plot_ID_corner = NULL, tree_coord = NULL, plot_ID_tree = NULL) {
  ...
  divide_plot_fct <- function(dat, grid_size) {
    ...
    if((max(dat[["x_rel"]])-min(dat[["x_rel"]])) %% grid_size[1] != 0) {
      stop("The x-dimension of the plot is not a multiple of the x-dimension of the grid size")
    }
    if((max(dat[["y_rel"]])-min(dat[["y_rel"]])) %% grid_size[2] != 0) {
    ...
    plot_grid <- data.table(as.matrix(expand.grid(
      x_rel = seq(min(dat[["x_rel"]]), max(dat[["x_rel"]]), by = grid_size[1]),
      y_rel = seq(min(dat[["y_rel"]]), max(dat[["y_rel"]]), by = grid_size[2])
    )))
    ...
    plot_grid <- rbindlist(apply(plot_grid[x_rel < max(x_rel) & y_rel < max(y_rel),], 1, function(grid_dat) {
      ...
      plot_grid[
        ...
        .(subplot_id = paste(plot_ID_corner, (X-min(plot_grid$x_rel)) / grid_size[1], (Y-min(plot_grid$y_rel)) / grid_size[2], sep = "_"),x_rel, y_rel)]
    }))
  ...
  if(!is.null(tree_coord)) {
    ...
    invisible(lapply(split(sub_corner_coord, by = "subplot_id", keep.by = TRUE), function(dat) {
      tree_coord[ plot_id == dat$plot_ID_corner[1] &
                   ...
                   get(names(tree_coord)[2]) %between% range(dat[["y_rel"]]),
