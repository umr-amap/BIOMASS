...
#' @param tree_data a data frame containing tree relative coordinates and other tree metrics (one row per tree).
...
#' tree_data <- data.frame(x_tree = runif(50,0,200), y_tree = runif(50,0,200))
...
#' tree_data <- rbind(tree_data, data.frame(x_tree = runif(50,0,200), y_tree = runif(50,0,200)))
#' corner_plot_ID <- rep(c("plot1","plot2"), e=4)
#' tree_plot_ID <- rep(c("plot1","plot2"), e=50)
#' subplots <- divide_plot(rel_coord, proj_coord, 100,
#'                         tree_data, c("x_tree","y_tree"),
#'                         corner_plot_ID, tree_plot_ID)
#'
 
divide_plot <- function(rel_coord, proj_coord = NULL, grid_size, tree_data = NULL, tree_coords = NULL, corner_plot_ID = NULL, tree_plot_ID = NULL, grid_tol = 0.1, centred_grid = F) {
  ...
  if(!is.null(tree_data) && (!is.matrix(tree_data) & !is.data.frame(tree_data))){
    stop("tree_data must be a matrix or a data frame")
  }
  ...
  if(nrow(rel_coord)!=4 & !is.null(tree_data) & is.null(tree_plot_ID)){
    stop("You must supply tree_plot_ID if you have more than one plot in your data")
  }
  if(!is.null(tree_data) && !is.null(tree_plot_ID) && nrow(tree_data)!=length(tree_plot_ID)) {
    stop("The length of tree_plot_ID and the number of rows of tree_data are different")
  }
  ...
  if(!is.null(tree_data)) {
    ...
    if(!is.null(tree_plot_ID)){
      tree_data[,plot_id:=tree_plot_ID]
    } else{
      tree_data[,plot_id:=""]
    } 
