...
if (!file.exists('data-raw/genList_tpl.csv')){
  ...
  tplList = data.table( family = tplList[class == "family", fam], 
...
# take apg families
if (!file.exists("apgFamilies.csv")) source("data-raw/apgfamily.R")
...
tplList[!is.na(famAPG), family := famAPG]

tplList[, ':='(order = NULL, famAPG = NULL, class = NULL)]
...
kew[!is.na(famAPG), family := famAPG]
kew[, ':='(order = NULL, famAPG = NULL, num = NULL, smthg = NULL, Author = NULL, numAgain = NULL)]
...
write.csv(dup, "data-raw/dupGenusToSub.csv", quote = F, col.names = F)

if(!file.exists("data-raw/dupGenus_TNRS.txt"))
  stop("Please submite the file 'dupGenusToSub.csv' to this site:\n\t\t",
...
setDT(BIOMASS::genusFamily)[!genusFamily, on = "genus"]
genusFamily[!setDT(BIOMASS::genusFamily), on = "genus"]
