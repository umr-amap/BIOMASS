...
lm_model <- modelHD(D =  NouraguesHD$D, H = NouraguesHD$H, method = "log2", useWeight = FALSE, drawGraph = T, bayesian = FALSE)
lm_model$model
lm_model$RSElog

lm_model_weight <- modelHD(D =  NouraguesHD$D, H = NouraguesHD$H, method = "log2", useWeight = TRUE, drawGraph = T, bayesian = FALSE)
lm_model_weight$model
lm_model_weight$RSElog
exp(predict(lm_model_weight$model , newdata = data.frame(D = 50)))
exp(0.51695 + 1.14188 * log(50) + -0.09854 * log(50)^2)

brm_model <- modelHD(D =  NouraguesHD$D, H = NouraguesHD$H, method = "log2", useWeight = FALSE, drawGraph = T, bayesian = T, useCache = TRUE, cores = 3)
# saveRDS(brm_model, file = "~/Documents/saves/brm_models/brm_model.rds")
# brm_model <- readRDS(file = "~/Documents/saves/brm_models/log2/brm_model.rds")
brm_model$model
brm_model$RSElog
brm_model_weight <- modelHD(D =  NouraguesHD$D, H = NouraguesHD$H, method = "log2", useWeight = TRUE, drawGraph = T, bayesian = T, useCache = TRUE, cores = 3)
# saveRDS(brm_model_weight, file = "~/Documents/saves/brm_models/brm_model_weight.rds")
# brm_model_weight <- readRDS(file = "~/Documents/saves/brm_models/log2/brm_model_weight.rds")
brm_model_weight$model
brm_model_weight$RSElog
verif_df <- data.frame(D = 10:100)
coef_brm_model <- fixef(brm_model_weight$model)[,1]
verif_df <- verif_df |> 
  mutate(pred_fun = exp(predict(brm_model_weight$model , newdata = verif_df)[,1]), 
         pred_formula_log2 = exp(coef_brm_model[1] + coef_brm_model[2] * log(D) + coef_brm_model[3] * log(D)^2))
verif_df |> pivot_longer(c("pred_fun","pred_formula_log2")) |> ggplot(aes(x=D,y=value,col=name)) + geom_point() + theme_minimal()
...
plot(H_lm_model$H , exp(predict(lm_model$model, newdata = data.frame(D=NouraguesTrees$D)) + 0.5*lm_model$RSElog^2)) ; abline(a = c(0,1), col="red") # OK !
plot(H_brm_model$H , exp(predict(brm_model$model, newdata = data.frame(D=NouraguesTrees$D))[,1] + 0.5*brm_model$RSElog^2)) ; abline(a = c(0,1), col="red") # OK !
plot(H_brm_model_weighted$H , exp(predict(brm_model_weight$model, newdata = data.frame(D=NouraguesTrees$D))[,1] + 0.5*brm_model_weight$RSElog^2)) ; abline(a = c(0,1), col="red") # OK !
plot(H_lm_model$H , H_lm_model_new$H) ; abline(a = c(0,1)) # OK !
plot(H_lm_model$H , H_brm_model$H) ; abline(a = c(0,1)) # OK !
plot(H_brm_model$H , H_brm_model_weighted$H) ; abline(a = c(0,1)) # OK !


# Testing with stand-specific models
forest_inv <- read.csv("data-raw/forest_inv_exemple.csv")
lm_model_plots <- modelHD(D =  forest_inv$D, H = forest_inv$H, method = "log2", useWeight = FALSE, drawGraph = T, plot = forest_inv$Plot)
lm_model_plots_weighted <- modelHD(D =  forest_inv$D, H = forest_inv$H, method = "log2", useWeight = TRUE, drawGraph = T, plot = forest_inv$Plot)
brm_model_plots <- modelHD(D =  forest_inv$D, H = forest_inv$H, method = "log2", useWeight = FALSE, drawGraph = T, bayesian = T, useCache = NULL, plot = forest_inv$Plot, cores = 3)
# saveRDS(brm_model_plots, file = "~/Documents/saves/brm_models/brm_model_plots.rds")
# brm_model_plots <- readRDS(file = "~/Documents/saves/brm_models/log2/brm_model_plots.rds")
brm_model_plots_weighted <- modelHD(D =  forest_inv$D, H = forest_inv$H, method = "log2", useWeight = TRUE, drawGraph = T, bayesian = T, useCache = TRUE, plot = forest_inv$Plot, cores = 3)
# saveRDS(brm_model_plots_weighted, file = "~/Documents/saves/brm_models/log2/brm_model_plots_weighted.rds")
# brm_model_plots_weighted <- readRDS(file = "~/Documents/saves/brm_models/log2/brm_model_plots_weighted.rds")
