...
lm_model <- modelHD(D =  NouraguesHD$D, H = NouraguesHD$H, method = "michaelis", useWeight = FALSE, drawGraph = T, bayesian = FALSE)
plot(lm_model$model)
lm_model$RSE

lm_model_weight <- modelHD(D =  NouraguesHD$D, H = NouraguesHD$H, method = "michaelis", useWeight = TRUE, drawGraph = T, bayesian = FALSE)
lm_model_weight$model
lm_model_weight$RSE
predict(lm_model_weight$model , newdata = data.frame(D = 50))
coefficients(lm_model_weight$model)
52.3014 * 50 / (29.2561 + 50)

brm_model <- modelHD(D = NouraguesHD$D, H = NouraguesHD$H,
                     method = "michaelis", bayesian = TRUE, useCache = TRUE,
                     useWeight = FALSE, drawGraph = T, cores = 3)
# saveRDS(brm_model, file = "~/Documents/saves/brm_models/michaelis/brm_model.rds")
# brm_model <- readRDS(file = "~/Documents/saves/brm_models/michaelis/brm_model.rds")
brm_model$model
brm_model$RSE
...
# brm_model_weight <- readRDS(file = "~/Documents/saves/brm_models/michaelis/brm_model_weight.rds")
...
verif_df <- verif_df |> 
  mutate(pred_fun = predict(brm_model_weight$model , newdata = verif_df)[,1] ,
         pred_formula_michaelis = coef_brm_model[1] * D / (D + coef_brm_model[2]))
verif_df |> pivot_longer(c("pred_fun","pred_formula_michaelis")) |> ggplot(aes(x=D,y=value,col=name)) + geom_point() + theme_minimal()

H_lm_model <- retrieveH(D = NouraguesTrees$D, model = lm_model)
H_lm_model_new <- retrieveH(D = NouraguesTrees$D, model = lm_model)
...
plot(H_lm_model$H , H_lm_model_new$H) ; abline(a = c(0,1), col="red") # OK !
...
lm_model_plots <- modelHD(D =  forest_inv$D, H = forest_inv$H, method = "michaelis", useWeight = FALSE, drawGraph = T, plot = forest_inv$Plot)
lm_model_plots_weighted <- modelHD(D =  forest_inv$D, H = forest_inv$H, method = "michaelis", useWeight = TRUE, drawGraph = T, plot = forest_inv$Plot)
brm_model_plots <- modelHD(D =  forest_inv$D, H = forest_inv$H, method = "michaelis", useWeight = FALSE, drawGraph = T, bayesian = T, useCache = NULL, plot = forest_inv$Plot, cores = 3)
# saveRDS(brm_model_plots, file = "~/Documents/saves/brm_models/michaelis/brm_model_plots.rds")
# brm_model_plots <- readRDS(file = "~/Documents/saves/brm_models/michaelis/brm_model_plots.rds")
brm_model_plots_weighted <- modelHD(D =  forest_inv$D, H = forest_inv$H, method = "michaelis", useWeight = TRUE, drawGraph = T, bayesian = T, useCache = TRUE, plot = forest_inv$Plot, cores = 3)
# saveRDS(brm_model_plots_weighted, file = "~/Documents/saves/brm_models/michaelis/brm_model_plots_weighted.rds")
# brm_model_plots_weighted <- readRDS(file = "~/Documents/saves/brm_models/michaelis/brm_model_plots_weighted.rds")
...
H_lm_model_plots_new <- retrieveH(D = forest_inv$D, model = lm_model_plots, plot = forest_inv$Plot)
plot(H_lm_model_plots$H , H_lm_model_plots_new$H) ; abline(a = c(0,1), col="red") # OK !
