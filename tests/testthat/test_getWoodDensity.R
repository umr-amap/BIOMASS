...
test_that("Function getWoodDensity", {
  ...
  expect_error( suppressMessages(getWoodDensity(taxo$genusCorrected, taxo$speciesCorrected[1:10])), 
  ...
  expect_error( suppressMessages( getWoodDensity(genus=taxo$genusCorr, species=taxo$speciesCorr, 
                              stand=KarnatakaForest$plotId, addWoodDensityData = LocalWoodDensity) ), 
...
test_that("Function getWoodDensity with other parameters", {
  ...
  expect_error( suppressMessages(getWoodDensity(taxo$genusCorrected, taxo$speciesCorrected, region = "feqz")), 
  ...
  expect_true(any( is.na( suppressMessages(getWoodDensity(taxo$genusCorrected, 
                                                          taxo$speciesCorrected, region = "SouthAmericaTrop")$family)) ))
  
  expect_error(suppressMessages(getWoodDensity(taxo$genusCorrected, taxo$speciesCorrected, family = taxo$family[1:10])), 
               "same length")
  
  expect_failure(expect_equal(suppressMessages(getWoodDensity(taxo$genusCorrected, taxo$speciesCorrected, family = taxo$family)),
                              suppressMessages(getWoodDensity(taxo$genusCorrected, taxo$speciesCorrected))))
  
  expect_error(suppressMessages(getWoodDensity(taxo$genusCorrected, taxo$speciesCorrected, stand = KarnatakaForest$plotId[1:10])), 
               "same length")
  
  expect_failure(expect_equal(suppressMessages(getWoodDensity(taxo$genusCorrected, 
                                                              taxo$speciesCorrected, stand = KarnatakaForest$plotId)),
                              suppressMessages(getWoodDensity(taxo$genusCorrected, taxo$speciesCorrected))))
})
