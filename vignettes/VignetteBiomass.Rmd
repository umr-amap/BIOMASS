...
result <- modelHD(D=NouraguesHD$D, 
                   H =NouraguesHD$H,
                   drawGraph=TRUE,
                   useWeight=TRUE)
kable(result)
...
ResHD = t(rbind( sapply( HDmodelPerPlot, function(x) coef(x$model)), RSE = RSEmodels ))
kable(ResHD, row.names = F, digits = 3)
```
...
resultMC<-by(KarnatakaForestsub, KarnatakaForestsub$plotId,
             ...
             simplify=F)
meanAGBperplot<-sapply(resultMC, function(x) x$meanAGB)
credinf = sapply(resultMC, function(x) x$credibilityAGB[1])
credsup<-sapply(resultMC, function(x) x$credibilityAGB[2])
ord<-order(meanAGBperplot)
plot(meanAGBperplot[ord],pch=20,xlab="Plots",ylab="AGB (Mg/ha)",ylim=c(0,max(credsup)),las=1,cex.lab=1.3)
segments(1:length(ord),credinf[ord],1:length(ord),credsup[ord],col="red")
```
