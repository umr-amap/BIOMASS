...
* calculating the projected/geographic coordinates of the plot's corners and trees from the relative coordinates (those of the field) 

* validating plot's corners and trees with LiDAR data
...
* a data frame of trees coordinates, containing at least :
  ...
  - the tree coordinates in the relative system (the field coordinates)
...
* The GPS coordinates of the plot corners are considered very accurate or enough measurements have been made to be confident in the accuracy of their mean. In this case, when the relative coordinates of subplots and trees will be projected in the projected/geographic system, these coordinates will follow the relationship between the plot corners coordinates in both systems.

* Too few measurements of the GPS coordinates of plot corners have been taken and/or are not reliable. In this case, the relative coordinates (the plot shape measured on the field) are considered to be accurate and the GPS corner coordinates will be recalculated to fit the relative coordinates.
...
### When relying on GPS corner measurements
...
Note that if you only have 4 GPS measurements, but these were acquired with high accuracy (by a geometer, for example), or if you have averaged your measurements by yourself, you can supply these 4 GPS coordinates to the function.
...
A few comments on the graph and outputs

### When relying on field measurements

```{r dpi=200}
correct_plot <- checkPlotCoord(
  longlat = cornerCoord[, c("Long", "Lat")],
  relCoord = cornerCoord[, c("xRel", "yRel")],
  trustGPScorners = F, cornerID = cornerCoord$Corners,
  drawPlot = TRUE,
  maxDist = 30, rmOutliers = TRUE
)

```


THE REST OF THE VIGNETTE HAS NOT BEEN DONE YET. IT IS JUST A FEW TESTS TO REMEMBER AND THAT WILL BE INCORPORATED INTO TESTHAT
