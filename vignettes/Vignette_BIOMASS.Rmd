...
Equation describing AGB allometry can be defined by the user, provided that **a sample of the distributions of corresponding model parameters is supplied**. For tropical forests, users can choose to model AGB allometry using Chave's equation calibrated on which data? (ref 2014), in this case **no parameter sample is needed**. The error propagation due to the **uncertainty of the model parameters** is then performed by the function. The propagation of the error due to the **uncertainty of the model variables** (D, WD and H) can be **parameterized by the user**.

## AGB allometry

- chave version: default, or allometry = chave2014, note that makes the codes written using previous versions of BIOMASS remain valid. it is the one used for the whole vignette.
