...
This vignette describes how to compute above ground biomass (AGB) and its associated uncertainty from plot inventory data. For the sake of clarity, the article follows the same workflow as in the BIOMASS paper, excluded the shaded area:

![BIOMASS workflow - Article Estimate biomass](img/BIOMASS_workflow_articles.png)
...
**Wood density** is estimated **from tree taxonomy**, using the [Global Wood Density Database v.2](https://zenodo.org/records/18262736) as a reference. So the first step might be to correct tree taxonomy.
...
You can also retrieve families from genus names in the familyAccepted column.
...
**Additional wood density values (mean and error) can be added** using the `addWoodDensityData` argument (here invented for the example):
...
As the log2 model has the lowest RSE, we will build this model using the `method` argument and add its predictions to the dataset with the `retrieveH()` function:
...
By default, `bayesian = TRUE` in `modelHD()` arguments. In this case, a Bayesian model will be fitted using the brms package (see section [Building bayesian Height-Diameter models](#bayesian-models)), ensuring proper propagation of parameter uncertainties. As this method is more time-consuming, for educational purposes, we will set bayesian = FALSE.

Note that if some of the trees' heights had been measured in the `NouraguesTrees` dataset, we could have provided these heights to `modelHD()`. In this case, we could also have created a model for each stand/sub-region present in `NouraguesTrees` using the `plot` argument. But keep in mind that even for well-measured trees, `retrieveH()` gives you the predicted height (see [this section](#mixing-measured_estimated)).
...
The `AGBmonteCarlo()` function allows the user to propagate different sources of error up to the final AGB estimate. The propagation of the error due to the **uncertainty of the model variables** (D, WD and H) can be **parameterized by the user**. The propagation of the error due to the **uncertainty of the allometry model parameters** is automatically performed by the function. 
...
Note that when HDmodel is not a bayesian model, uncertainties of the parameters are not propagated during the error propagation of the AGBmonteCarlo() function.
...
## AGB visualisation and plot level summary

Let's propagate diameter, wood density and height errors, using the HD-model for height:
...
# Provide your own AGB allometry

## Default allometry or user allometry?

For tropical forests, users can choose to model AGB allometry using eqn 4
of [Chave et al. (2014)](https://onlinelibrary.wiley.com/doi/abs/10.1111/gcb.12629). In that case you do not need to provide a value to the `allometry` argument, default value being `"chave2014"`. Hence any code written using previous versions of BIOMASS remains valid if you update to v3.1 and above. Note that the entire BIOMASS vignette uses Chave's AGB allometry equation.

However, if you have an allometry dataset (*e.g.* TLS measurements, destructive AGB) you can use it to calibrate the allometric model, then use the calibrated model as the allometry equation in `AGBmonteCarlo`. The error due uncertainty in model parameters will still be propagated by the function. The easiest way is to pass to the `allometry` argument the output of a `brms` fit. Two examples are shown below.

## First example 

[Takoudjou et al. 2017 MEE](https://doi.org/10.1111/2041-210X.12933) data: calibrate user volume allometry with TLS data (DBH, H, volume) using brms, compute volume with destructive data (DBH, H), and compare with destructive volume from the dataset.

```{r, user_allom_takoudjou_ex}
#1) LOAD DATA
tree_data_raw <- readxl::read_excel("saved_data/database_takoudjou_paper.xls", sheet = 2)
tree_data <- tree_data_raw %>% 
  select(c('ID', 'Genus', 'Species', 'DBH_dest', 'H_tot_dest', 'Destructive total volume',
           'DBH_L', 'Hauteur_L', 'TLS edited total volume_5')) %>%
  mutate_at(c('DBH_dest', 'H_tot_dest', 'Destructive total volume',
              'DBH_L', 'Hauteur_L', 'TLS edited total volume_5'), as.numeric) %>%
  rename(Vol_tls = 'TLS edited total volume_5') %>%
  rename(Vol_dest = 'Destructive total volume')

#2) CALIBRATE VOLUME ALLOMETRY WITH TLS DATA
tree_data_allom <- tree_data %>%
  select(c('ID', 'DBH_L', 'Hauteur_L', 'Vol_tls'))

# plot predictor vs. predicted
tree_data_allom$predictor <- log(tree_data_allom$DBH_L^2 * tree_data_allom$Hauteur_L) 
tree_data_allom$logV <- log(tree_data_allom$Vol_tls)
ggplot(tree_data_allom, aes(x=predictor, y=logV)) +
  geom_point()+
  theme_minimal()

# write allometry equation as formula
allom_formula <- bf(logV ~ a + b * log(Hauteur_L * DBH_L^2), a ~ 1, b ~ 1, nl = TRUE)

# model fit & checks
fit_allom <- brm(formula = allom_formula, 
                 data = tree_data_allom,
                 family = gaussian(),
                 thin = 10, control = list(max_treedepth = 13), iter = 3500, warmup = 1500, chains = 3, 
                 refresh = 0, silent = 2)
plot(fit_allom)
summary(fit_allom)
pp_check(fit_allom, ndraws = 150)

#3) VALIDATION WITH DESTRUCTIVE VOLUME
# compute volume with allometry uncertainty only 
predV <- AGBmonteCarlo(n = 1000,
                         D = tree_data$DBH_dest, Dpropag = NULL,
                         H = tree_data$H_tot_dest, errH = 0,
                         fitted_allom = fit_allom, var_in_data = c("logV", "H", "D"),
                         volume_allom = T, return_volume = T
)

# plot predicted Volume vs observed destrucTive Volume
tree_data_pred <- tree_data %>%
  mutate(predV_median = predV$V_simu_per_stem$medV,
         lower_CI_V = predV$V_simu_per_stem$lower_CI_V,
         upper_CI_V = predV$V_simu_per_stem$upper_CI_V)

ggplot(tree_data_pred, aes(x= Vol_dest, y=predV_median)) +
  geom_abline(intercept = 0, slope = 1, lty = 2, colour = "grey")+
  theme_minimal()+
  geom_segment(aes(x = Vol_dest, y = lower_CI_V, 
                   xend = Vol_dest, yend = upper_CI_V))+
  geom_point()+
  theme(legend.position = "none")+
  xlab("Observed destructive Volume (m^3)")+
  ylab("Predicted Volume (m^3)")
```

## Second example 

[Schepaschenko et al. 2017 PANGAEA](https://doi.pangaea.de/10.1594/PANGAEA.871491150) data: select one species (P. *sylvestris* L.) in one biome (Temperate Grasslands, Savannas, and Shrublands), dataset partition for calibration (70%) and validation (30%), calibrate AGB allometry with calibration data (DBH, H, destructive AGB) using brms, compute AGB for validation data (DBH, H) and compare with observed AGB.

```{r, user_allom_Schepaschenko_ex}
#1) LOAD & PREPARE DATA
tree_data_raw <- suppressWarnings(readxl::read_xlsx("saved_data/Biomass_tree_DB_Scientific_data_2017.xlsx"))
tree_data <- tree_data_raw %>% 
  select(c('ID', 'Species', 'DBH', 'H tree', 'Pabo', 'Location', 'Latitude', 'Longitude',
           'Ecoregion', 'ID_Plot')) %>%
  rename(H = 'H tree') %>%
  rename(D = DBH) %>%
  rename(AGB = Pabo) %>%
  filter(Species == "Pinus sylvestris L.") %>%
  filter(!str_starts(Ecoregion, "808"))  %>%
  filter(D >=5) %>%  
  filter(!is.na(H)) %>%
  filter(!is.na(AGB)) %>%
  mutate(logAGB = log(AGB))

# get wood density
woodDens <- getWoodDensity(genus = "Pinus", species = "Pinus sylvestris L.")
tree_data$WD <- woodDens$meanWD
tree_data$sdWD <- woodDens$sdWD

#2) SPATIAL PARTITION OF THE DATASET for calibration & validation
set.seed(5678)
nb_cluster <- 4
cluster <- stats::kmeans(x = as.matrix(tree_data[,c('Latitude', 'Longitude')]), centers = nb_cluster)
plot(tree_data$Latitude, tree_data$Longitude, col = rainbow(nb_cluster)[cluster$cluster])
table(cluster$cluster)

# choosing cluster 2 for validation
sum(table(cluster$cluster)[c(2)])/sum(table(cluster$cluster))
tree_data$cluster <- cluster$cluster
tree_data_cal <- tree_data %>%
  filter(cluster != 2)
tree_data_val <- tree_data %>%
  filter(cluster == 2)

#3) CALIBRATE AGB ALLOMETRY WITH CALIBRATION DATA
# plot predictor vs. predicted
tree_data_cal$predictor <- log(tree_data_cal$D^2 * tree_data_cal$H * tree_data_cal$WD) 
ggplot(tree_data_cal, aes(x=predictor, y=logAGB, colour = as.factor(cluster))) +
  geom_point()+
  theme_minimal()

# write allometry equation as formula
allom_formula <- bf(logAGB ~ a + b * log(WD * H * D^2), a ~ 1, b ~ 1, nl = TRUE)
...
#4) VALIDATION
# compute AGB with allometry uncertainty only for validation data
predAGB <- AGBmonteCarlo(n = 1000,
                         D = tree_data_val$D, Dpropag = NULL,
                         H = tree_data_val$H, errH = 0,
                         WD = tree_data_val$WD, errWD = 0,
                         fitted_allom = fit_allom, var_in_data = c("logAGB", "WD", "H", "D")
)

# plot predicted AGB vs. observed AGB
tree_data_val_pred <- tree_data_val %>%
  mutate(predAGB_median = predAGB$AGB_simu_per_stem$medAGB,
         lower_CI_AGB = predAGB$AGB_simu_per_stem$lower_CI_AGB,
         upper_CI_AGB = predAGB$AGB_simu_per_stem$upper_CI_AGB)

ggplot(tree_data_val_pred, aes(x=AGB, y=predAGB_median, colour = as.factor(Location))) +
  geom_abline(intercept = 0, slope = 1, lty = 2, colour = "grey")+
  scale_color_brewer(palette = "Dark2")+
  theme_minimal()+
  geom_segment(aes(x = AGB, y = lower_CI_AGB, xend = AGB, yend = upper_CI_AGB, colour = Location))+
  geom_point()+
  #theme(legend.position = "none")+
  xlab("Observed AGB (Mg)")+
  ylab("Predicted AGB (Mg)")
```

# Some tricks

## Mixing measured and estimated height values {#mixing-measured_estimated}
...
## Building bayesian Height-Diameter models {#bayesian-models}

Since version 2.2.6, BIOMASS enables to build bayesian H-D models using the [brms package](https://paulbuerkner.com/brms/).

As compiling the Stan programme may take some time the first time around, we recommend setting the 'useCache' argument to TRUE in the `modelHD()` function. This will save the model as a .rds file in the defined cache path, meaning that next time, the model will simply be loaded and updated, bypassing the compilation stage.

```{r log2_bayesian, eval=FALSE}
...
If the **Michaelis-Menten or Weibull methods are used**, you must pay close attention to the returned models as the algorithm is likely to identify numerous **local minima for the estimated parameters**. Here is an example of the Weibull method being used with the default arguments: 

```{r weibull_bayesian, eval=FALSE}
brm_model <- modelHD(
  D = NouraguesHD$D, H = NouraguesHD$H,
  method = "weibull",
  bayesian = TRUE, useCache = TRUE)
...
As one can see, the two first parameters a and b (resp. called `b_a_Intercept` and `b_b_Intercept`) of the Weibull equation: $$H=a  \cdot (1-exp(-(D/b)^c))$$ are strongly correlated. Therefore, it is strongly recommended to define priors that do not allow for unrealistic values. To do this, we need to examine the meanings of the Weibull parameters:

- `a` can be considered as the hypothetical maximum height, so defining a uniform prior between 0 and 80m seems realistic.
- `b` is the scaling parameter, so defining a uniform prior between 0 and a high value of D (e.g. the 90th percentile) makes sense.
- `c` is the shape parameter which must be between 0 and 1

Besides, since model parameters and chain iterations are strongly correlated, an increase of `thin`, `iter` and `warmup` is required (see the help of brms::brm() and [this page](https://mc-stan.org/learn-stan/diagnostics-warnings.html) for more details).

The arguments of `brms::brm()` (cited above) can be provided to `modelHD()`:

```{r weibull_priors, eval=FALSE}
weibull_priors <- c(
