...
By setting `bayesian = TRUE` in `modelHD()` arguments, we could have built bayesian models using the brms package (see section [Building bayesian Height-Diameter models](#bayesian-models)). 
...
- `a` can be considered as the hypothetical maximum height, so defining a non-informative (uniform) prior between 0 and 80m seems realistic.
- `b` is the scaling parameter, so defining a non-informative (uniform) prior between 0 and a high value of D makes sens
