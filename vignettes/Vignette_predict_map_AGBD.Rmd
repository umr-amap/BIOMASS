...
summary(model_cal)
```
- sdgp(betatilde_gpxy), lscale(betatilde_gpxy), betatilde_Intercept, sigma 
...
This posterior predictive check enables to quickly visualise if the distribution of observed data `y`, blue bold line, matches the distributions of data predicted by the calibrated model `y_rep`, light blue lines - one per parameter draw or combination.
...
Parallelisation of `predict_map` function is handled by the `future` framework. So to compute the map predictions in parallel you only need to add
