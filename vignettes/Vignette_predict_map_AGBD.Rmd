...
ggplot(dt_plot_val, aes( x = median_obs, y = AGBD_pred.Estimate, colour = subplot_ID))+ 
  geom_point(shape = 1)+
  geom_segment(aes(y = AGBD_pred.Estimate, yend = AGBD_pred.Estimate, x = inf_obs, xend = sup_obs))+
  geom_segment(aes(y = AGBD_pred.Q2.5, yend = AGBD_pred.Q97.5, x = median_obs, xend = median_obs))+
  geom_abline(colour = "red", linetype = 2, linewidth = 1.2)+
  xlab("Observed log AGBD")+
  ylab("Predicted log AGBD")
```

Uncertainties on the prediction are represented by the vertical segments, while the original uncertainties on data, namely subplot level AGBD, are represented by the horizontal segments. When you are good with your validation results, you infer the model **with the entire dataset**, and then predict a robust AGBD map.
