---
title: "Spatialized trees and forest stand metrics with BIOMASS"
...
output: 
  prettydoc::html_pretty: 
    ...
    theme: cayman
vignette: >
  %\VignetteIndexEntry{Spatialized trees and forest stand metrics with BIOMASS}
...
* calculating the projected/geographic coordinates of the plot's corners and the trees from the relative coordinates (or local coordinates, ie, those of the field) 
...
Two data frames are required for the rest. One for the corner coordinates of the plot(s), and another for the trees coordinates. 

* The **corner coordinates data frame**, must contains at least:
...
In this vignette, for educational purposes, we will not use only one but **two datasets of corner coordinates**, derived from permanent plots in the Nouragues forest (French Guiana): 
...
* The **trees coordinates data frame**, must contains at least:
  - the name of the plots if there are several plots
...
This dataset is also derived from the [2012 Nouragues forest dataset](https://dataverse.cirad.fr/dataset.xhtml?persistentId=doi:10.18167/DVN1/TZ1RL9), but for educational purposes, some virtual trees have been added in the data.
...
check_plot_trust_GPS <- check_plot_coord(
  ...
  trust_GPS_corners = T, corner_ID = "CornerID",
  draw_plot = TRUE,
...
check_plot_trust_field <- check_plot_coord(
  ...
  longlat = c("Long", "Lat"),  # or proj_coord = c("Xutm", "Yutm"), 
  rel_coord = c("Xfield", "Yfield"),
  trust_GPS_corners = FALSE, corner_ID = "CornerID",
...
The associated polygon is returned via the `$polygon` output and can be saved into a shapefile like this: 

```{r, eval=FALSE}
sf::st_write(check_plot_trust_GPS$polygon, "your_directory/plot201.shp")
```

For full details, the `$outlier_corners` output returns all the information about GPS outliers found by the function, and the `$UTM_code` output returns the UTM code calculated by the function if geographic coordinates have been supplied.
...
check_plot_trust_GPS <- check_plot_coord(
  corner_data = NouraguesPlot201,
  longlat = c("Long", "Lat"), rel_coord = c("Xfield", "Yfield"), corner_ID = "CornerID",
...
kable(head(check_plot_trust_GPS$tree_data), digits = 3, row.names = FALSE, caption = "Head of the $tree_data output")
```
...
You can also get and modify the plot via the `$plot_design` output which is a ggplot object. For example, to change the plot title: 
...
Be aware that by default, the function will ask you to type Enter between each plot (ask = TRUE).
...
The function can handle as many plots as you want, using the `corner_plot_ID` and `tree_plot_ID` arguments:
...
Once you've applied the `divide_plot()` function with a non-null `tree_data` argument, you can summarise any tree metric present in divide_plot()$tree_data at the subplot level with the `subplot_summary()` function.
...
**By default, the function sums the metric per subplot** and divide the result by the area of each subplot (to obtain a summary **per hectare**) , but you can request any valid function with the `fun` argument and choose between a raw or a per hectare summary with the `per_ha` argument.
...
subplot_polygons <- sf::st_set_crs(subplot_metric$polygon , value = "EPSG:2972")

# Save the polygons in a shapefile
sf::st_write(subplot_polygons, "your_directory/subplots_201.shp")
```

And of course, the function can handle **as many plots as supplied in divide_plot()**:
