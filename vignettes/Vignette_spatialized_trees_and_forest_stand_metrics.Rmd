...
Two data-frames are required for the rest. One for the corner of the plot(s), and one for the trees, containing at least their coordinates. 
...
In this vignette, for educational purposes, we will not use only one but **two datasets of corner coordinates**, derived from permanent plots in the [Nouragues forest](https://dataverse.cirad.fr/dataset.xhtml?persistentId=doi:10.18167/DVN1/TZ1RL9) (French Guiana): 
...
* The data-set including the **tree coordinates**, must contain at least:
...
This dataset is also derived from the [2012 Nouragues forest dataset](https://dataverse.cirad.fr/dataset.xhtml?persistentId=doi:10.18167/DVN1/TZ1RL9), but for educational purposes, some virtual trees with erroneous coordinates have been added in the data.
...
* **Too few measurements of the GPS coordinates** of plot corners have been taken and/or are **not reliable**. In this case, **the shape of the plot measured on the field is considered to be accurate** and the GPS corner coordinates will be recalculated to fit the shape and dimensions of the plot.  See [3.1.2](#trustPlotShape)
...
Dividing plots into several sub-plots is performed using the `divide_plot()` function. This function takes the relative coordinates of the 4 corners of the plot to divide it into a grid. Be aware that **the plot must be rectangular in the plot's relative coordinates system**, ie, have 4 right angles:
...
If you want to stay in the plot's relative coordinate system, just let `proj_coord` = NULL.
