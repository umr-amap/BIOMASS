...
```{r}
trees <- trees %>% mutate(check_plot_trust_GPS$treeProjCoord)
```


You can also grep and modify the plot via the `$plotDesign` output which is a ggplot object. For example, to change the plot title : 
...
foo <- function(x) {return(mean(x,na.))}
